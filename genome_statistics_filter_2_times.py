...
after 1st filtering contigs: /home/zhanghao/graminearum/filter_contigs/filtered_genomes/180201_filtered.fasta

after 2nd filtering contigs: /home/zhanghao/graminearum/filter_2nd_contigs/filtered_2nd_genomes/180201_filtered_2nd.fas
...
def genome_statistics(strain_name_list):
    for strain_name in strain_name_list:
        ...
        cmd_1st_filtering = 'fasta_assembly_statistics /home/zhanghao/graminearum/filter_1st_contigs/filtered_genomes/{0}_filtered.fasta > /home/zhanghao/graminearum/genomes_statistics/after_filtering_1st/{0}_1st_filtering.txt'.format(strain_name)
        subprocess.check_call(cmd_1st_filtering, shell=True)

        cmd_2nd_filtering = 'fasta_assembly_statistics /home/zhanghao/graminearum/filter_2nd_contigs/filtered_2nd_genomes/{0}_filtered_2nd.fas > /home/zhanghao/graminearum/genomes_statistics/after_filtering_2nd/{0}_2nd_filtering.txt'.format(strain_name)
        subprocess.check_call(cmd_2nd_filtering, shell=True)

        print(strain_name + ': 4 fasta_assembly_statistics run successfully')
...
def parse_1st_filtering(strain_name):
    filtering_list = []
    with open('/home/zhanghao/graminearum/genomes_statistics/after_filtering_1st/' + strain_name + '_1st_filtering.txt', 'r') as f:
        for line in f:
            line = line.strip()
            if line.startswith('Number of contigs'):
                line = line.split(':\t')
...
def parse_2nd_filtering(strain_name):
    filtering_list = []
    with open('/home/zhanghao/graminearum/genomes_statistics/after_filtering_2nd/' + strain_name + '_2nd_filtering.txt', 'r') as f:
        for line in f:
            line = line.strip()
            if line.startswith('Number of contigs'):
                line = line.split(':\t')
...
def main(strain_name_list):
    ...
    with open('/home/zhanghao/graminearum/genomes_statistics/genome_statistics_result_289.csv', 'w') as f:
        f.write('strain_name\tRaw_Total_reads\tRaw_Total_bases\tRaw_Q20_bases\tRaw_Q30_bases\tRaw_GC_content\t-\tQC_Total_reads\tQC_Total_bases\tQC_Q20_bases\tQC_Q30_bases\tQC_GC_content\t-\tSpades_Number_of_contigs\tSpades_Total_size(bp)\tSpades_Mean_contig_size(bp)\tSpades_Shortest_contig(bp)\t-\tDecon_Number_of_contigs\tDecon_Total_size(bp)\tDecon_Mean_contig_size(bp)\tDecon_Shortest_contig(bp)\t-\t1st_Filter_Number_of_contigs\t1st_Filter_Total_size(bp)\t1st_Filter_Mean_contig_size(bp)\t1st_Filter_Shortest_contig(bp)\t-\t2nd_Filter_Number_of_contigs\t2nd_Filter_Total_size(bp)\t2nd_Filter_Mean_contig_size(bp)\t2nd_Filter_Shortest_contig(bp)\t-\t1st_Removed_length_percentage(%)\t-\t2nd_Removed_length_percentage(%)\n')
        for strain_name in strain_name_list:
            qc = parse_qc(strain_name)
            spades = parse_spades(strain_name)
            decontamination = parse_decontamination(strain_name)
            filtering_1st = parse_1st_filtering(strain_name)
            filtering_2nd = parse_2nd_filtering(strain_name)
            removed_percentage_1st = format((int(decontamination[1]) - int(filtering_1st[1]))/int(decontamination[1]) * 100, '.2f')
            removed_percentage_2nd = format((int(filtering_1st[1]) - int(filtering_2nd[1]))/int(filtering_1st[1]) * 100, '.2f')

            f.write(strain_name + '\t' + qc[0] + '\t' + qc[1] + '\t' + qc[2] + '\t' + qc[3] + '\t' + qc[4] + '\t-\t' + qc[5] + '\t' + qc[6] + '\t' + qc[7] + '\t' + qc[8] + '\t' + qc[9] + '\t-\t' + spades[0] + '\t' + spades[1] + '\t' + spades[2] + '\t' + spades[3] + '\t-\t' + decontamination[0] + '\t' + decontamination[1] + '\t' + decontamination[2] + '\t' + decontamination[3] + '\t-\t' + filtering_1st[0] + '\t' + filtering_1st[1] + '\t' + filtering_1st[2] + '\t' + filtering_1st[3] + '\t-\t' + filtering_2nd[0] + '\t' + filtering_2nd[1] + '\t' + filtering_2nd[2] + '\t' + filtering_2nd[3] + '\t-\t' + removed_percentage_1st + '\t-\t' + removed_percentage_2nd + '\n')
