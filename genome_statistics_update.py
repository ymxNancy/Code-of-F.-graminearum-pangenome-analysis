...
def genome_statistics(strain_name_list):
    for strain_name in strain_name_list:
        ...
        cmd_ordered = 'fasta_assembly_statistics /home/zhanghao/graminearum/order_contigs/ordered_genomes/{0}_ordered.fas > /home/zhanghao/graminearum/genomes_statistics/after_ordered/{0}_ordered.txt'.format(strain_name)
        subprocess.check_call(cmd_ordered, shell=True)
...
def parse_ordered(strain_name):
    ordered_list = []
    with open('/home/zhanghao/graminearum/genomes_statistics/after_ordered/' + strain_name + '_ordered.txt', 'r') as f:
        for line in f:
            line = line.strip()
            if line.startswith('Number of contigs'):
                line = line.split(':\t')
                ordered_list.append(line[1])
            elif line.startswith('Total size'):
                line = line.split(':\t')
                ordered_list.append(line[1])
            elif line.startswith('Mean contig size'):
                line = line.split(':\t')
                ordered_list.append(format(float(line[1]), '.2f'))
            elif line.startswith('Shortest contig'):
                line = line.split(':\t')
                ordered_list.append(line[1])
        return ordered_list
...
def main(strain_name_list):
    ...
    with open('/home/zhanghao/graminearum/genomes_statistics/genome_statistics_result_271.csv', 'w') as f:
        f.write('strain_name\tRaw_Total_reads\tRaw_Total_bases\tRaw_Q20_bases\tRaw_Q30_bases\tRaw_GC_content\t-\tQC_Total_reads\tQC_Total_bases\tQC_Q20_bases\tQC_Q30_bases\tQC_GC_content\t-\tSpades_Number_of_contigs\tSpades_Total_size(bp)\tSpades_Mean_contig_size(bp)\tSpades_Shortest_contig(bp)\t-\tDecon_Number_of_contigs\tDecon_Total_size(bp)\tDecon_Mean_contig_size(bp)\tDecon_Shortest_contig(bp)\t-\tOrdered_Number_of_contigs\tOrdered_Total_size(bp)\tOrdered_Mean_contig_size(bp)\tOrdered_Shortest_contig(bp)\t-\tRemoved_length_percentage(%)\n')
        for strain_name in strain_name_list:
            qc = parse_qc(strain_name)
            spades = parse_spades(strain_name)
            decontamination = parse_decontamination(strain_name)
            ordered = parse_ordered(strain_name)
            removed_percentage = format((int(decontamination[1]) - int(ordered[1]))/int(decontamination[1]) * 100, '.2f')
            f.write(strain_name + '\t' + qc[0] + '\t' + qc[1] + '\t' + qc[2] + '\t' + qc[3] + '\t' + qc[4] + '\t-\t' + qc[5] + '\t' + qc[6] + '\t' + qc[7] + '\t' + qc[8] + '\t' + qc[9] + '\t-\t' + spades[0] + '\t' + spades[1] + '\t' + spades[2] + '\t' + spades[3] + '\t-\t' + decontamination[0] + '\t' + decontamination[1] + '\t' + decontamination[2] + '\t' + decontamination[3] + '\t-\t' + ordered[0] + '\t' + ordered[1] + '\t' + ordered[2] + '\t' + ordered[3] + '\t-\t' + removed_percentage + '\n')
