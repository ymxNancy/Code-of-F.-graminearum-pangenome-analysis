...
def parse_minimap2(strain_name, ref_file, minimap_result_file):
    cmd = f'minimap2 -xasm5 {ref_file} /home/zhanghao/graminearum/sidr_analyses/sidr_genomes/{strain_name}_sidr.fasta > {minimap_result_file}'
...
def get_threshold(gc_content_file):
    with open(gc_content_file, 'r') as f:
        all_gc_content = []
        for line in f:
            line = line.strip().split()
            if line[1] != 'GC_rate':
                all_gc_content.append(float(line[1]))

        #calculating standard deviation of the dataset
        std = st.pstdev(all_gc_content)
        total_average = mean(all_gc_content)
        return std, total_average

def get_lower_coverage_perc_contigs(candidate_contigs_file, gc_content_dir, std_num, gc_average, coverage_threshold=80):
    ...
    with open(candidate_contigs_file, 'r') as f:
        for line in f:
            ...
            if line[2] != 'coverage_perc' and float(line[2]) <= coverage_threshold:
                ...
                min_gc = gc_average - 2*std_num
                max_gc = gc_average + 2*std_num
...
def get_short_length_contigs(strain_name):
    cmd = f'fasta_length /home/zhanghao/graminearum/sidr_analyses/sidr_genomes/{strain_name}_sidr.fasta -o > {strain_name}_ctg_contig_length.txt'
    check = subprocess.check_call(cmd, shell=True)
    if check == 0:
...
def get_mt_contigs(mt_candidate_contigs_file):
    ...
    with open(mt_candidate_contigs_file, 'r') as f:
        for line in f:
            line = line.strip().split()
            if line[0] != 'contig_identifier' and float(line[1]) >= 70:
...
def filter_genome(removed_contigs, strain_name):
    ...
    strain_dir = {}
    with open('/home/zhanghao/graminearum/sidr_analyses/sidr_genomes/{0}_sidr.fasta'.format(strain_name), 'r') as f1, open('/home/zhanghao/graminearum/filter_contigs/filtered_genomes/{0}_filtered.fasta'.format(strain_name), 'w') as f2:
        for line in f1:
            line = line.strip()
            if line.startswith('>'):
                key = line[1:]
...
if __name__ == "__main__":
    ...
    threshold = get_threshold(gc_content_file)
    print('Success: ' + strain_name + ' std is ' + str(threshold[0]))
    print('Success: ' + strain_name + ' total_average_gc is ' + str(threshold[1]))
    getted_lower_coverage_perc_contigs = get_lower_coverage_perc_contigs(candidate_contigs_file, getted_gc_content_dir, threshold[0], threshold[1], coverage_threshold=80)
